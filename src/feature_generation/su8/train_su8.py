#!/usr/bin/env python
"""SU8 特徴量バンドルの学習エントリーポイント。

本スクリプトは SU1+SU5+GroupImputers パイプラインに SU8 ボラティリティ・レジーム特徴を追加し、
LightGBM 回帰器を学習する。生成された ``sklearn.Pipeline`` は
``artifacts/SU8/inference_bundle.pkl`` に保存され、推論時に同じ処理フローを再利用できる。
...
from src.feature_generation.su8.feature_su8 import (  # noqa: E402
    SU8Config,
    SU8FeatureAugmenter,
    load_su8_config,
)
...
class SU8FullFeatureAugmenter(BaseEstimator, TransformerMixin):
    """SU1 + SU5 + SU8 特徴量を入力フレームへ追加するトランスフォーマー。

    パイプラインの位置づけ:
    生データ → SU1 → SU5 → GroupImputers → **SU8** → 前処理 → モデル

    Note: SU8 は GroupImputers 通過後の特徴行列に適用するため、
    本クラスは SU1+SU5 の生成後、GroupImputers の後で SU8 を適用する設計。
    ただし sklearn Pipeline の制約上、ここでは SU1+SU5 を先に適用し、
    その後パイプラインで GroupImputers → SU8Augmenter の順で処理する。
    """
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "SU8FullFeatureAugmenter":
    ...
    def transform(
        ...
        if not hasattr(self, "su1_generator_"):
            raise RuntimeError(
                "SU8FullFeatureAugmenter must be fitted before transform()."
            )
    ...
    def _ensure_dataframe(X: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(X, pd.DataFrame):
            raise TypeError("SU8FullFeatureAugmenter expects a pandas.DataFrame input")
        return X.copy()
...
def build_pipeline(
    ...
    *,
    ...
    """SU1+SU5+GroupImputers+SU8 パイプラインを構築する。"""
    augmenter = SU8FullFeatureAugmenter(su1_config, su5_config, fill_value=numeric_fill_value)
    su8_augmenter = SU8FeatureAugmenter(su8_config, fill_value=numeric_fill_value)
    ...
    # Pipeline: SU1+SU5 → GroupImputers → SU8 → preprocess → model
    steps = [
        ("augment", augmenter),
        ("m_imputer", m_imputer),
        ("e_imputer", e_imputer),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Train a LightGBM model with SU8 volatility/regime features."
    )
    ap.add_argument(
        "--data-dir",
        type=str,
    ...
    ap.add_argument("--out-dir", type=str, default="artifacts/SU8")
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
        default=0.0,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    su8_config = load_su8_config(args.config_path)
    ...
    su8_prefit = SU8FullFeatureAugmenter(
    ...
    X_augmented_all = su8_prefit.transform(X_np, fold_indices=fold_indices_full)
    ...
    su8_augment_step = named_steps.get("su8_augment")
    ...
    su8_generated_columns = (
        list(getattr(su8_augment_step, "su8_feature_names_", []))
        if su8_augment_step is not None
        else []
    )
    feature_manifest: Dict[str, Any] = {
        ...
        "su8_generated_columns": su8_generated_columns,
    }
    feature_manifest["su1_generated_columns_cv"] = getattr(
        su8_prefit, "su1_feature_names_", []
    )
    feature_manifest["su5_generated_columns_cv"] = getattr(
        su8_prefit, "su5_feature_names_", []
    )
    ...
    su8_config_serialized = _normalise_scalar(asdict(su8_config))
    ...
    if not args.no_artifacts:
        ...
        meta = {
            ...
            "su8_config": su8_config_serialized,
            ...
            "su8_feature_count": len(feature_manifest.get("su8_generated_columns", [])),
