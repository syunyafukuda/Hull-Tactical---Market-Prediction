#!/usr/bin/env python
"""SU10 モデルの推論エントリーポイント。

学習済みの ``artifacts/SU10/inference_bundle.pkl`` を読み込み、指定されたテストデータに
対して推論を実行し、提出形式の ``submission.csv``（および parquet 版）を出力する。

Kaggle APIモード推論機能も提供する。
"""
...
from src.feature_generation.su10.train_su10 import SU10FeatureAugmenter  # noqa: E402,F401
...
def _check_bundle_compat(meta: Mapping[str, Any]) -> None:
    ...
    if isinstance(preprocess_hash, str) and isinstance(preprocess_path_str, str):
        ...
        if preprocess_path.exists():
            ...
            print(f"[warn] unable to verify preprocess config hash; file not found: {preprocess_path}")


def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run inference with the SU10 pipeline bundle.")
    parser.add_argument("--data-dir", type=str, default="data/su10", help="Directory containing raw datasets")
    parser.add_argument("--test-file", type=str, default=None, help="Explicit path to the test file")
    parser.add_argument("--artifacts-dir", type=str, default="artifacts/SU10", help="Directory storing SU10 artifacts")
    parser.add_argument("--bundle-path", type=str, default=None, help="Override path to inference_bundle.pkl")
    parser.add_argument("--meta-path", type=str, default=None, help="Override path to model_meta.json")
    parser.add_argument("--feature-list-path", type=str, default=None, help="Override path to feature_list.json")
    parser.add_argument("--su10-path", type=str, default=None, help="Path to su10_external_regime.csv for Kaggle API mode")
    parser.add_argument("--id-col", type=str, default=None, help="ID column name (defaults to metadata value)")
    parser.add_argument("--pred-col", type=str, default="prediction", help="Output prediction column name")
    parser.add_argument("--out-parquet", type=str, default="artifacts/SU10/submission.parquet", help="Submission parquet output path")
    parser.add_argument("--out-csv", type=str, default="artifacts/SU10/submission.csv", help="Submission csv output path")
    parser.add_argument("--no-csv", action="store_true", help="Suppress CSV output and write parquet only")
    parser.add_argument(
        "--max-missing-columns",
        type=int,
        default=20,
    ...
    parser.add_argument("--clip-low", type=float, default=None, help="Optional lower bound applied to predictions")
    parser.add_argument("--clip-high", type=float, default=None, help="Optional upper bound applied to predictions")
...
def _post_process_predictions(
    ...
    *,
    ...
    if not np.isfinite(prediction).all():
        if ensure_finite:
            raise ValueError("Predictions contain non-finite values and --ensure-finite is set.")
...
def predict_kaggle_api(
    ...
    ----------
    test : pd.DataFrame
        Kaggle API から提供されるテストデータ（date_id を含む）
    bundle_path : str | Path
        inference_bundle.pkl のパス
    su10_path : str | Path
        su10_external_regime.csv のパス
    meta : Mapping[str, Any] | None
        model_meta.json の内容（オプション）
    ...
    float
        予測値（単一のシグナル値）
    """
    test_pd = test if isinstance(test, pd.DataFrame) else pd.DataFrame(test)

    # SU10 特徴量を結合
    su10 = pd.read_csv(su10_path) if isinstance(su10_path, (str, Path)) else su10_path
    test_merged = test_pd.merge(su10, on="date_id", how="left")
    ...
    if meta is None:
        meta = {}
    raw_pred = _predict_with_task(pipe, test_merged, meta)

    # シグナル変換
    postprocess_params = _resolve_postprocess_params(meta)
    signal = to_signal(raw_pred, postprocess_params)

    # 単一値を返す
    return float(signal[0]) if len(signal) > 0 else 1.0
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(f"inference bundle not found: {bundle_path}. Run train_su10.py first.")
    ...
    postprocess_params = _resolve_postprocess_params(meta if isinstance(meta, Mapping) else {})
    print(f"[info] post-process params: mult={postprocess_params.mult}, lo={postprocess_params.lo}, hi={postprocess_params.hi}")
    ...
    # SU10 特徴量の結合（Kaggle API モードの場合）
    if args.su10_path:
        su10_path = Path(args.su10_path)
        if su10_path.exists():
            print(f"[info] loading SU10 features from: {su10_path}")
            su10_df = pd.read_csv(su10_path)
            test_df = test_df.merge(su10_df, on="date_id", how="left")
            print(f"[info] merged SU10 features: {len([c for c in test_df.columns if c.startswith('su10_')])} columns")
    ...
    missing_calendar_cols = [col for col in required_calendar_cols if col not in test_df.columns]
    ...
    try:
        ...
        raise ValueError("Unable to interpret 'is_scored' column as boolean mask.") from exc
    ...
    if not np.isfinite(submission_scored[pred_col]).all():
        raise ValueError("Submission contains non-finite predictions after signal transformation.")
