#!/usr/bin/env python
"""SU10 特徴量バンドルの学習エントリーポイント。

本スクリプトは SU1 + SU5 + SU10 特徴量を使用して LightGBM 回帰器を学習する。
SU10 は外部データ（SPY Historical Data）から生成された特徴量で、
train_with_su10.csv に事前に結合されている。

主な役割
--------
* SU1/SU5 用 YAML 設定を読み込む。
* train_with_su10.csv を読み込み、SU10 特徴量を含むデータで学習。
* :class:`SU5FeatureAugmenter` で SU1 + SU5 特徴量を追加。
...
class SU10FeatureAugmenter(BaseEstimator, TransformerMixin):
    """SU1 + SU5 特徴量を入力フレームへ追加するトランスフォーマー。

    SU10 特徴量は事前に train_with_su10.csv に結合されているため、
    このクラスでは SU1 + SU5 のみを生成する。
    """
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "SU10FeatureAugmenter":
    ...
    def transform(
        ...
        if not hasattr(self, "su1_generator_"):
            raise RuntimeError("SU10FeatureAugmenter must be fitted before transform().")
        ...
        # Concatenate: original + SU1 + SU5 (SU10 is already in original)
    ...
    def _ensure_dataframe(X: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(X, pd.DataFrame):
            raise TypeError("SU10FeatureAugmenter expects a pandas.DataFrame input")
        return X.copy()
...
def infer_train_file(data_dir: Path, explicit: str | None) -> Path:
    if explicit:
        ...
        return candidate
    for name in ("train_with_su10.csv", "train.parquet", "train.csv", "Train.parquet", "Train.csv"):
...
def infer_test_file(data_dir: Path, explicit: str | None) -> Path:
    if explicit:
        ...
        return candidate
    for name in ("test_with_su10.csv", "test.parquet", "test.csv", "Test.parquet", "Test.csv"):
        candidate = data_dir / name
        if candidate.exists():
            return candidate
    for ext in ("parquet", "csv"):
...
def build_pipeline(
    ...
    *,
    ...
    augmenter = SU10FeatureAugmenter(su1_config, su5_config, fill_value=numeric_fill_value)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Train a LightGBM model with SU1+SU5+SU10 features.")
    ap.add_argument("--data-dir", type=str, default="data/su10", help="Directory containing train/test files (default: data/su10)")
    ...
    ap.add_argument("--out-dir", type=str, default="artifacts/SU10")
...
def _prepare_features(
    ...
    *,
    ...
    SU10 特徴量は train_with_su10.csv に含まれているため、そのまま使用される。
    """
    drop_cols = {
        target_col,
        id_col,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Check if SU10 features are present in train data
    su10_cols_in_train = [c for c in train_df.columns if c.startswith("su10_")]
    if not su10_cols_in_train:
        print("[warn] No SU10 columns found in train data. Please use train_with_su10.csv.")
    else:
        print(f"[info] Found {len(su10_cols_in_train)} SU10 columns in train data")
    ...
    su10_prefit = SU10FeatureAugmenter(su1_config, su5_config, fill_value=args.numeric_fill_value)
    su10_prefit.fit(X_np)
    ...
    X_augmented_all = su10_prefit.transform(X_np, fold_indices=fold_indices_full)
    ...
    su10_columns_in_input = [c for c in feature_cols if c.startswith("su10_")]
    feature_manifest: Dict[str, Any] = {
        ...
        "su10_input_columns": su10_columns_in_input,
    }
    feature_manifest["su1_generated_columns_cv"] = getattr(su10_prefit, "su1_feature_names_", [])
    feature_manifest["su5_generated_columns_cv"] = getattr(su10_prefit, "su5_feature_names_", [])
    ...
    if not args.no_artifacts:
        ...
        meta = {
            ...
            "feature_count_before_augment": len(feature_cols),
            ...
            "su10_feature_count": len(su10_columns_in_input),
