#!/usr/bin/env python
"""SU1 特徴量バンドルの学習エントリーポイント。

本スクリプトは生データから欠損構造を表現する SU1 特徴量を生成し、基礎列と結合した
うえで軽量な前処理パイプラインを通し、LightGBM 回帰器を学習する。生成された
``sklearn.Pipeline``（特徴量生成＋前処理＋モデル）は
``artifacts/su1/inference_bundle.pkl`` に保存され、推論時に同じ処理フローを再利用できる。

主な役割
--------
* SU1 用 YAML 設定を読み込む。
* :class:`SU1FeatureGenerator` で SU1 特徴量を作成し、生データの説明変数へ連結する。
* 時系列分割で OOF 指標を算出し、挙動を記録する。
* 全学習データで再学習したパイプラインやメタ情報、特徴量リストを成果物として出力する。

既存の学習スクリプト（例: ``train_simple.py``）と同じ CLI 操作感を意識しており、
オーケストレーション側の扱いを統一しやすい構成になっている。
"""
...
try:  # LightGBM を既定モデルとして使用するため、未インストール時は明示的に失敗させる
	from lightgbm import LGBMRegressor  # type: ignore
	import lightgbm as lgb  # type: ignore

	HAS_LGBM = True
except Exception:  # pragma: no cover - ガード用分岐
...
def _ensure_numpy_bitgenerator_aliases() -> None:
	...
	try:
		import numpy as _np  # limited scope import for alias setup
	except Exception:  # pragma: no cover - defensive fallback when numpy is missing
		return
	...
	# Fallback: ensure the canonical key exists even if registries were not found.
...
def _normalise_scalar(value: Any) -> Any:
	...
	if value is pd.NA:  # type: ignore[attr-defined]
		return None
	# pandas の拡張 dtype など JSON 化できないオブジェクトは名前で記録する
...
def _build_preprocess(num_fill_value: float, *, handle_unknown: str = "ignore") -> ColumnTransformer:
	...
	try:  # sklearn 1.2 以降の API
		encoder = OneHotEncoder(sparse_output=False, **encoder_kwargs)
	except TypeError:  # pragma: no cover - sklearn 1.2 未満向けのフォールバック
...
def build_pipeline(
	...
	*,
	...
	augmenter = SU1FeatureAugmenter(config, fill_value=numeric_fill_value)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
	...
	ap.add_argument("--out-dir", type=str, default="artifacts/SU1")
...
def _initialise_callbacks(model: Any) -> List[Any]:
	...
	except Exception:  # pragma: no cover - コールバック連携の任意処理
		callbacks = []
	return callbacks
...
def main(argv: Sequence[str] | None = None) -> int:
	...
	config = load_su1_config(args.config_path)
	...
	su1_prefit = SU1FeatureAugmenter(config, fill_value=args.numeric_fill_value)
	su1_prefit.fit(X_np)
	X_augmented_all = su1_prefit.transform(X_np)
	...
	feature_manifest["su1_generated_columns_cv"] = getattr(su1_prefit, "su1_feature_names_", [])
	...
	su1_config_serialized = _normalise_scalar(asdict(config))
