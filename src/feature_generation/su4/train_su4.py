#!/usr/bin/env python
"""SU4 特徴量バンドルの学習エントリーポイント。

本スクリプトは生データから SU1 特徴量を生成し、GroupImputers で補完した上に
SU4 代入影響トレース特徴量を追加して前処理パイプラインを通し、LightGBM 回帰器を学習する。
生成された ``sklearn.Pipeline``（特徴量生成＋前処理＋モデル）は
``artifacts/SU4/inference_bundle.pkl`` に保存され、推論時に同じ処理フローを再利用できる。

主な役割
--------
* SU1/SU4 用 YAML 設定を読み込む。
* :class:`SU1FeatureAugmenter` + GroupImputers + :class:`SU4FeatureAugmenter` で特徴量を作成。
...
class SU1FeatureAugmenter(BaseEstimator, TransformerMixin):
	...
	def transform(self, X: pd.DataFrame) -> pd.DataFrame:
		...
		augmented = pd.concat([frame, su1_features], axis=1)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
	ap = argparse.ArgumentParser(description="Train a LightGBM model with SU4 features.")
	...
	ap.add_argument("--out-dir", type=str, default="artifacts/SU4")
...
def main(argv: Sequence[str] | None = None) -> int:
	...
	X_np, y_np, feature_cols = _prepare_features(
		train_df, test_df,
	...
	print(f"[info] train samples: {len(X_np)}, target mean={y_np.mean():.6f}, std={y_np.std():.6f}")
	...
	callbacks = _initialise_callbacks(model_step)
	...
	su1_step = named_steps.get("su1")
	su4_step = named_steps.get("su4")
	...
	su4_config_serialized = _normalise_scalar(asdict(su4_config))
