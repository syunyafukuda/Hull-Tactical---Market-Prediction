...
学習済みの ``artifacts/SU9/inference_bundle.pkl`` を読み込み、指定されたテストデータに
...
from src.feature_generation.lagged.feature_su5 import (  # noqa: E402,F401
    SU5Config,
    SU5FeatureGenerator,
)
from src.feature_generation.su9.feature_su9 import (  # noqa: E402,F401
    SU9Config,
    SU9FeatureAugmenter,
    SU9FeatureGenerator,
)
from src.feature_generation.su9.train_su9 import (  # noqa: E402,F401
...
DEFAULT_POSTPROCESS_PARAMS = PostProcessParams(mult=1.0, lo=0.9, hi=1.1)
...
def _resolve_postprocess_params(
    ...
    *,
    ...
    """Resolve post-process params for SU9.
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run inference with the SU9 pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        "--artifacts-dir",
        type=str,
        default="artifacts/SU9",
        help="Directory storing SU9 artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
    ...
    parser.add_argument(
        "--out-parquet",
        type=str,
        default="artifacts/SU9/submission.parquet",
        help="Submission parquet output path",
    )
    parser.add_argument(
        "--out-csv",
        type=str,
        default="artifacts/SU9/submission.csv",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run train_su9.py first."
        )
