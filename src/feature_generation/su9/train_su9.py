#!/usr/bin/env python
"""SU9 特徴量バンドルの学習エントリーポイント。

本スクリプトは SU1+SU5+GroupImputers パイプラインに SU9 カレンダー・季節性特徴を追加し、
LightGBM 回帰器を学習する。生成された ``sklearn.Pipeline`` は
``artifacts/SU9/inference_bundle.pkl`` に保存され、推論時に同じ処理フローを再利用できる。
...
from src.feature_generation.su9.feature_su9 import (  # noqa: E402
    SU9Config,
    SU9FeatureAugmenter,
    load_su9_config,
)
...
class SU9FullFeatureAugmenter(BaseEstimator, TransformerMixin):
    """SU1 + SU5 + SU9 特徴量を入力フレームへ追加するトランスフォーマー。

    パイプラインの位置づけ:
    生データ → SU1 → SU5 → GroupImputers → **SU9** → 前処理 → モデル

    Note: SU9 は GroupImputers 通過後の特徴行列に適用するため、
    本クラスは SU1+SU5 の生成後、GroupImputers の後で SU9 を適用する設計。
    ただし sklearn Pipeline の制約上、ここでは SU1+SU5 を先に適用し、
    その後パイプラインで GroupImputers → SU9Augmenter の順で処理する。
    """
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "SU9FullFeatureAugmenter":
    ...
    def transform(
        ...
        if not hasattr(self, "su1_generator_"):
            raise RuntimeError(
                "SU9FullFeatureAugmenter must be fitted before transform()."
            )
        ...
        su5_features = self.su5_generator_.transform(
            su1_features, fold_indices=fold_indices
        )
    ...
    def _ensure_dataframe(X: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(X, pd.DataFrame):
            raise TypeError("SU9FullFeatureAugmenter expects a pandas.DataFrame input")
        return X.copy()
...
def load_preprocess_policies(config_path: str | Path) -> Dict[str, Dict[str, Any]]:
    ...
    def _policy_params(section: Mapping[str, Any]) -> Dict[str, Any]:
        raw_params = (
            section.get("policy_params", {}) if isinstance(section, Mapping) else {}
        )
    ...
    groups = {
        key: full_cfg.get(key, {}) or {}
        for key in ("m_group", "e_group", "i_group", "p_group", "s_group")
    }
    ...
    for key, section in groups.items():
        ...
        if key == "e_group":
            ...
            common["clip_quantile_high"] = float(
                section.get("clip_quantile_high", 0.999)
            )
            ...
            if disable_flag is not None:
                ...
                common["enable_quantile_clip"] = bool(
                    section.get("enable_quantile_clip", True)
                )
            ...
            common["fallback_quantile_low"] = float(
                section.get("fallback_quantile_low", 0.005)
            )
            common["fallback_quantile_high"] = float(
                section.get("fallback_quantile_high", 0.995)
            )
    return settings
...
def _collect_imputer_metadata(pipeline: Pipeline) -> Dict[str, Dict[str, Any]]:
    ...
    for step_name in ("m_imputer", "e_imputer", "i_imputer", "p_imputer", "s_imputer"):
        ...
        step_meta: Dict[str, Any] = {
            "policy_requested": getattr(
                imputer, "policy_requested", getattr(imputer, "policy", None)
            ),
            ...
            "calendar_column": getattr(
                imputer,
                "_calendar_column_name_",
                getattr(imputer, "calendar_column", None),
            ),
            "policy_params": _normalise_scalar(
                getattr(imputer, "_policy_params", getattr(imputer, "policy_params", {}))
            ),
...
def _build_preprocess(
    ...
    # SU9 flag columns are uint8 and don't need this handling.
...
def build_pipeline(
    ...
    *,
    ...
    """SU1+SU5+GroupImputers+SU9 パイプラインを構築する。"""
    augmenter = SU9FullFeatureAugmenter(su1_config, su5_config, fill_value=numeric_fill_value)
    su9_augmenter = SU9FeatureAugmenter(su9_config, fill_value=numeric_fill_value)
    ...
    if not HAS_LGBM or LGBMRegressor is None:  # type: ignore[truthy-function]
        raise RuntimeError(
            "LightGBM is required but not installed. Please install 'lightgbm'."
        )
    model = LGBMRegressor(**model_kwargs)  # type: ignore[arg-type]

    # Pipeline: SU1+SU5 → GroupImputers → SU9 → preprocess → model
    steps = [
        ("augment", augmenter),
        ("m_imputer", m_imputer),
        ("e_imputer", e_imputer),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Train a LightGBM model with SU9 calendar/seasonality features."
    )
    ap.add_argument(
        "--data-dir",
        type=str,
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Explicit path to the training file",
    )
    ap.add_argument(
    ...
    ap.add_argument("--out-dir", type=str, default="artifacts/SU9")
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
        default=0.0,
        help="Value used to fill numeric NaNs after feature generation",
    )
    ap.add_argument(
        "--n-splits", type=int, default=5, help="Number of folds for TimeSeriesSplit"
    )
    ap.add_argument(
        "--gap",
        type=int,
        default=0,
        help="Gap between train and validation indices in each fold",
    )
    ap.add_argument(
        "--min-val-size",
        type=int,
        default=0,
        help="Skip folds where validation after gap is smaller than this size",
    )
    ...
    ap.add_argument(
        "--verbosity", type=int, default=-1, help="LightGBM verbosity level"
    )
    ap.add_argument(
        "--no-artifacts",
        action="store_true",
        help="If set, do not write artifacts to disk",
    )
    ap.add_argument(
        "--signal-optimize-for",
        type=str,
    ...
    *,
    ...
    if exclude_lagged:
        use_cols = [
            ...
            if c in test_cols and not c.startswith("lagged_")
        ]
    else:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    su9_config = load_su9_config(args.config_path)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        if len(train_idx) == 0 or len(val_idx) == 0:
            print(
                f"[warn][fold {fold_idx}] skipped due to empty train/val after gap application"
            )
            continue
        if args.min_val_size and len(val_idx) < args.min_val_size:
            print(
                f"[warn][fold {fold_idx}] skipped because validation size {len(val_idx)} < min-val-size"
            )
            continue
        ...
        if signal_optimize_for == "vmsr":
            candidates = [
                ...
                if row["mult"] == fold_params.mult
                and row["lo"] == fold_params.lo
                and row["hi"] == fold_params.hi
            ]
            if candidates:
        ...
        fold_metrics = evaluate_msr_proxy(
            pred, y_valid.to_numpy(), fold_params, eps=signal_eps, lam=fold_lam
        )
    ...
    if valid_mask.any():
        ...
        if signal_optimize_for == "vmsr":
            candidates = [
                row
                for row in grid_all
                if row["mult"] == best_params_global.mult
                and row["lo"] == best_params_global.lo
                and row["hi"] == best_params_global.hi
            ]
            if candidates:
                best_row = max(candidates, key=lambda r: r.get("vmsr", float("-inf")))
        ...
        if signal_optimize_for == "vmsr":
            print(
                f"[metric][signal] vmsr={best_metrics_global['vmsr']:.6f} lam={lam_best_global:.2f}"
            )
    else:
    ...
    su9_augment_step = named_steps.get("su9_augment")
    model_step = named_steps.get("model")
    if model_step is None:
        raise RuntimeError("Pipeline is missing a 'model' step.")

    su1_generated_columns = (
        list(getattr(augment_step, "su1_feature_names_", []))
        if augment_step is not None
        else []
    )
    su5_generated_columns = (
        list(getattr(augment_step, "su5_feature_names_", []))
        if augment_step is not None
        else []
    )
    su9_generated_columns = (
        list(getattr(su9_augment_step, "su9_feature_names_", []))
        if su9_augment_step is not None
        else []
    )
    feature_manifest: Dict[str, Any] = {
        ...
        "su9_generated_columns": su9_generated_columns,
    }
    feature_manifest["su1_generated_columns_cv"] = getattr(
        su9_prefit, "su1_feature_names_", []
    )
    feature_manifest["su5_generated_columns_cv"] = getattr(
        su9_prefit, "su5_feature_names_", []
    )
    ...
    try:
        ...
        try:
            ...
            for step_name in ("m_imputer", "e_imputer", "i_imputer", "p_imputer", "s_imputer", "su9_augment"):
    ...
    su9_config_serialized = _normalise_scalar(asdict(su9_config))
    ...
    oof_best_metrics_serialized = {
        key: float(best_metrics_global[key]) for key in best_metric_keys
    }
    ...
    if not args.no_artifacts:
        ...
        meta = {
            ...
            "su9_config": su9_config_serialized,
            ...
            "su9_feature_count": len(feature_manifest.get("su9_generated_columns", [])),
        ...
        if valid_mask.any():
            ...
            oof_records = (
                {
                    ...
                    "fold": next(
                        (
                            ...
                            if log["val_start_index"] <= idx <= log["val_end_index"]
                        ),
                        None,
                    ),
                }
            ...
            _write_csv(
                oof_path, oof_records, fieldnames=["row_index", "y_true", "y_pred", "fold"]
            )
