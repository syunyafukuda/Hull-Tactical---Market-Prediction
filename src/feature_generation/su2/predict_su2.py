...
学習済みの ``artifacts/SU2/inference_bundle.pkl`` を読み込み、指定されたテストデータに
対して推論を実行し、提出形式の ``submission.csv``（および parquet 版）を出力する。

主な処理の流れ
----------------
1. 推論対象データセットを ``data/raw`` から自動検出（``--test-file`` で明示指定も可）。
2. 学習時に保存した ``feature_list.json`` から説明変数の列集合を取得し、テストデータを
   同じ列順に揃える（欠損列は NaN として追加する）。
3. ``joblib`` でパイプラインをロードし ``predict`` を実行。
4. ``date_id`` などの ID 列と予測値列で提出ファイルを生成する。
"""
...
def _ensure_numpy_bitgenerator_aliases() -> None:
	...
	except Exception:  # pragma: no cover - defensive guard when numpy is unavailable
		return

	generator_cls = getattr(_np.random, "MT19937", None)
	if generator_cls is None:  # pragma: no cover - extremely old numpy
		return
...
from src.feature_generation.su2.train_su2 import SU2FeatureAugmenter  # noqa: E402,F401
from src.feature_generation.su1.feature_su1 import SU1Config, SU1FeatureGenerator  # noqa: E402,F401
from src.feature_generation.su2.feature_su2 import SU2Config, SU2FeatureGenerator  # noqa: E402,F401
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run inference with the SU2 pipeline bundle.")
	parser.add_argument("--data-dir", type=str, default="data/raw", help="Directory containing raw datasets")
	parser.add_argument("--test-file", type=str, default=None, help="Explicit path to the test file")
	parser.add_argument("--artifacts-dir", type=str, default="artifacts/SU2", help="Directory storing SU2 artifacts")
	...
	parser.add_argument("--out-parquet", type=str, default="artifacts/SU2/submission.parquet", help="Submission parquet output path")
	parser.add_argument("--out-csv", type=str, default="artifacts/SU2/submission.csv", help="Submission csv output path")
