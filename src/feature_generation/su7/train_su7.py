#!/usr/bin/env python
"""SU7 特徴量バンドルの学習エントリーポイント。

本スクリプトは SU1+SU5+GroupImputers パイプラインに SU7 モメンタム・リバーサル特徴を追加し、
LightGBM 回帰器を学習する。生成された ``sklearn.Pipeline`` は
``artifacts/SU7/inference_bundle.pkl`` に保存され、推論時に同じ処理フローを再利用できる。
...
from src.feature_generation.su7.feature_su7 import (  # noqa: E402
    SU7Config,
    SU7FeatureAugmenter,
    load_su7_config,
)
...
class SU7FullFeatureAugmenter(BaseEstimator, TransformerMixin):
    ...
    パイプラインの位置づけ:
    生データ → SU1 → SU5 → GroupImputers → **SU7** → 前処理 → モデル

    Note: SU7 は GroupImputers 通過後の特徴行列に適用するため、
    本クラスは SU1+SU5 の生成後、GroupImputers の後で SU7 を適用する設計。
    ただし sklearn Pipeline の制約上、ここでは SU1+SU5 を先に適用し、
    その後パイプラインで GroupImputers → SU7Augmenter の順で処理する。
    """
    ...
    def fit(self, X: pd.DataFrame, y: Any = None) -> "SU7FullFeatureAugmenter":
    ...
    def transform(
        ...
        if not hasattr(self, "su1_generator_"):
            raise RuntimeError(
                "SU7FullFeatureAugmenter must be fitted before transform()."
            )
    ...
    def _ensure_dataframe(X: pd.DataFrame) -> pd.DataFrame:
        if not isinstance(X, pd.DataFrame):
            raise TypeError("SU7FullFeatureAugmenter expects a pandas.DataFrame input")
        return X.copy()
...
def build_pipeline(
    ...
    *,
    ...
    """SU1+SU5+GroupImputers+SU7 パイプラインを構築する。"""
    augmenter = SU7FullFeatureAugmenter(su1_config, su5_config, fill_value=numeric_fill_value)
    su7_augmenter = SU7FeatureAugmenter(su7_config, fill_value=numeric_fill_value)
    ...
    # Pipeline: SU1+SU5 → GroupImputers → SU7 → preprocess → model
    steps = [
        ("augment", augmenter),
        ("m_imputer", m_imputer),
        ("e_imputer", e_imputer),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Train a LightGBM model with SU7 momentum/reversal features."
    )
    ap.add_argument(
        "--data-dir",
        type=str,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    su7_prefit = SU7FullFeatureAugmenter(
    ...
    X_augmented_all = su7_prefit.transform(X_np, fold_indices=fold_indices_full)
    ...
    su7_augment_step = named_steps.get("su7_augment")
    ...
    su7_generated_columns = (
        list(getattr(su7_augment_step, "su7_feature_names_", []))
        if su7_augment_step is not None
        else []
    )
    feature_manifest: Dict[str, Any] = {
        ...
        "su7_generated_columns": su7_generated_columns,
    }
    feature_manifest["su1_generated_columns_cv"] = getattr(
        su7_prefit, "su1_feature_names_", []
    )
    feature_manifest["su5_generated_columns_cv"] = getattr(
        su7_prefit, "su5_feature_names_", []
    )
    ...
    su7_config_serialized = _normalise_scalar(asdict(su7_config))
    ...
    if not args.no_artifacts:
        ...
        meta = {
            ...
            "su7_config": su7_config_serialized,
            ...
            "su7_feature_count": len(feature_manifest.get("su7_generated_columns", [])),
