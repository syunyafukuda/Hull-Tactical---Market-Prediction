...
学習済みの ``artifacts/SU7/inference_bundle.pkl`` を読み込み、指定されたテストデータに
...
from src.feature_generation.su7.feature_su7 import (  # noqa: E402,F401
    SU7Config,
    SU7FeatureAugmenter,
    SU7FeatureGenerator,
)
from src.feature_generation.su7.train_su7 import (  # noqa: E402,F401
    SU7FullFeatureAugmenter,
)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run inference with the SU7 pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory storing SU7 artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
    ...
    parser.add_argument(
        "--out-parquet",
        type=str,
        default="artifacts/SU7/submission.parquet",
        help="Submission parquet output path",
    )
    parser.add_argument(
        "--out-csv",
        type=str,
        default="artifacts/SU7/submission.csv",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run train_su7.py first."
        )
