...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(
        description="Train LightGBM with FS_compact features."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--preprocess-config", type=str, default="configs/preprocess.yaml",
        help="Path to preprocess.yaml",
    )
    ap.add_argument(
        "--target-col", type=str, default="market_forward_excess_returns",
    )
    ap.add_argument(
        "--id-col", type=str, default="date_id",
    )
    ap.add_argument(
        "--out-dir", type=str, default="artifacts/models/lgbm",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--cv-mode", type=str, default="timeseries_split",
    ...
    ap.add_argument(
        "--wf-mode", type=str, default="expanding",
        choices=["expanding", "rolling"],
    )
    # Hull Sharpe evaluation
    ap.add_argument(
        "--eval-sharpe", action="store_true",
        help="Compute Hull Competition Sharpe in CV",
    )
    ap.add_argument(
        "--sharpe-mult", type=float, default=100.0,
        help="Multiplier for prediction->position mapping",
    )
    ap.add_argument(
        "--sharpe-offset", type=float, default=1.0,
        help="Offset for prediction->position mapping",
    )
...
def _compute_sharpe_for_fold(
    ...
    val_indices : np.ndarray
        Indices into train_df for validation set.
    sharpe_mult : float
        Multiplier for prediction->position mapping.
    sharpe_offset : float
        Offset for prediction->position mapping.
    ...
    # Compute Hull Sharpe
    result = compute_hull_sharpe(positions, forward_returns, risk_free_rate, validate=False)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.cv_mode == "walk_forward":
        ...
        print(f"[info] train_window={wf_config.train_window}, val_window={wf_config.val_window}, step={wf_config.step}")
    ...
    if not args.no_feature_exclusion:
        ...
        print(f"[info] Features: {n_before} -> {n_after} ({n_before - n_after} excluded)")
    else:
    ...
    cv_mode_display = "Walk-Forward" if args.cv_mode == "walk_forward" else "TimeSeriesSplit"
    print(f"\n{'='*60}")
    print(f"Starting {n_splits_actual}-fold {cv_mode_display} CV")
    if args.eval_sharpe:
        print(f"Hull Sharpe evaluation enabled (mult={args.sharpe_mult}, offset={args.sharpe_offset})")
    print(f"{'='*60}\n")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv_splits, start=1):
        ...
        if args.eval_sharpe:
            sharpe_dict = _compute_sharpe_for_fold(
                ...
                train_df=train_df,
                val_indices=val_idx,
                sharpe_mult=args.sharpe_mult,
                sharpe_offset=args.sharpe_offset,
            )
        ...
        if args.eval_sharpe and sharpe_dict:
            ...
            fold_log_entry["return_penalty"] = sharpe_dict.get("return_penalty", float("nan"))
    ...
    if valid_mask.any():
        oof_rmse = float(math.sqrt(mean_squared_error(
            y_np_array[valid_mask], oof_pred[valid_mask]
        )))
    ...
    if args.eval_sharpe and sharpe_results:
        ...
        oof_sharpe_summary = {
            ...
            "mean_vol_penalty": float(np.mean([r["vol_penalty"] for r in sharpe_results])),
            "mean_return_penalty": float(np.mean([r["return_penalty"] for r in sharpe_results])),
        }
    ...
    if not args.no_artifacts and not args.dry_run:
        ...
        inference_bundle = {
            ...
            "excluded_features": get_excluded_features(args.feature_tier) if not args.no_feature_exclusion else [],
        }
        ...
        print(f"[info] Bundle contains: augmenter + core_pipeline ({X_augmented_all.shape[1]} features)")
        ...
        if args.eval_sharpe and oof_sharpe_summary:
            sharpe_summary = {
                ...
                "config": {
                    "train_window": args.wf_train_window if args.cv_mode == "walk_forward" else None,
                    "val_window": args.wf_val_window if args.cv_mode == "walk_forward" else None,
        ...
        if args.eval_sharpe and oof_sharpe_summary:
            meta["oof_hull_sharpe_mean"] = oof_sharpe_summary.get("mean_sharpe", float("nan"))
            meta["oof_hull_sharpe_min"] = oof_sharpe_summary.get("min_sharpe", float("nan"))
