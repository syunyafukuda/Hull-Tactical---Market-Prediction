#!/usr/bin/env python
"""ExtraTrees training script using the unified model framework.

This script trains an ExtraTreesRegressor model using the FS_compact feature set (116 features)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="Train ExtraTrees with FS_compact features.")
    ap.add_argument(
        "--data-dir",
        type=str,
        default="data/raw",
    ...
    ap.add_argument(
        "--out-dir",
        type=str,
        default="artifacts/models/extratrees",
    )
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
    ...
    ap.add_argument("--max-features", type=float, default=0.7)
    ap.add_argument("--bootstrap", action="store_true", help="Enable bootstrap sampling")
...
def build_extratrees_pipeline(
    ...
    *,
    ...
    """Build ExtraTrees pipeline with SU1/SU5 augmenter and preprocessing.

    This is analogous to build_pipeline in train_su5.py but uses ExtraTreesRegressor
...
def build_extratrees_pipeline_no_augment(
    preprocess_settings: Dict[str, Any],
    *,
    numeric_fill_value: float,
    model_kwargs: Dict[str, Any],
    ...
    """Build ExtraTrees pipeline WITHOUT augmenter (for use with pre-augmented data).
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Build base pipeline with ExtraTrees
    base_pipeline = build_extratrees_pipeline(
        su1_config,
        su5_config,
        preprocess_settings,
    ...
    # Keep original column names (no sanitization needed for ExtraTrees)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        # Clone ExtraTrees model
        extratrees_model = clone(pipeline_steps["model"])

        # Fit
        extratrees_model.fit(X_train, y_train)  # type: ignore[union-attr]

        # Predict
        pred = extratrees_model.predict(X_valid)  # type: ignore[union-attr]
        pred = _to_1d(pred)

        # Compute metrics
        val_rmse = float(math.sqrt(mean_squared_error(y_valid, pred)))
        train_pred = extratrees_model.predict(X_train)  # type: ignore[union-attr]
    ...
    if not args.no_artifacts and not args.dry_run:
        ...
        # Use build_extratrees_pipeline_no_augment since X_augmented_all is already augmented
        print(f"[info] Training final model on {X_augmented_all.shape[1]} features (tier-excluded)")
        final_pipeline = build_extratrees_pipeline_no_augment(
            preprocess_settings,
            numeric_fill_value=args.numeric_fill_value,
            model_kwargs=model_kwargs,
            random_state=args.random_state,
        ...
        feature_list = {
            "version": f"extratrees-{args.feature_tier}-v1",
            "created_at": datetime.now(timezone.utc).isoformat(),
        ...
        if hasattr(final_model, "feature_importances_"):
            ...
            if len(model_input_columns) == len(importances):
                feature_names = model_input_columns
            else:
                # Fall back to generic names if mismatch
                feature_names = [f"feature_{i}" for i in range(len(importances))]
                print(f"[warn] Feature count mismatch: {len(model_input_columns)} columns vs {len(importances)} importances")
            ...
            print(f"[info] Saved feature importances to {importance_path}")
        ...
        # Filter to is_scored==True rows only
