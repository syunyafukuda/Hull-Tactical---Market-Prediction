...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with the ExtraTrees pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory storing ExtraTrees artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run train_extratrees.py first."
        )
    ...
    # ExtraTrees bundle is a direct Pipeline (not a dict)
