...
学習済みの ``artifacts/models/xgboost/inference_bundle.pkl`` を読み込み、指定されたテストデータに
対して推論を実行し、提出形式の ``submission.csv`` を出力する。

Usage:
    python -m src.models.xgboost.predict_xgb --artifacts-dir artifacts/models/xgboost
"""
...
def _resolve_postprocess_params(
    ...
    *,
    ...
    # Try oof_best_params first (XGBoost format), then post_process_params
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with the XGBoost pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory storing XGBoost artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run train_xgb.py first."
        )
    ...
    # XGBoost bundle is a direct Pipeline (not a dict)
