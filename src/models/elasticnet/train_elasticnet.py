#!/usr/bin/env python
"""ElasticNet training script using the unified model framework.

This script trains an ElasticNet model (L1+L2 regularization) using the FS_compact
feature set (116 features) and provides OOF evaluation consistent with other model types.

Key features:
- Uses FS_compact feature exclusion (tier3/excluded.json)
- Reuses existing preprocessing pipeline (M/E/I/P/S group imputers)
- Includes StandardScaler for feature normalization (required for linear models)
- Compatible with the unified CV evaluation framework
- Exports non-zero coefficients for interpretability
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="Train ElasticNet with FS_compact features.")
    ap.add_argument(
        "--data-dir",
        type=str,
        default="data/raw",
    ...
    ap.add_argument(
        "--out-dir",
        type=str,
        default="artifacts/models/elasticnet",
    )
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
    ...
    ap.add_argument("--alpha", type=float, default=0.001, help="Regularization strength")
    ap.add_argument("--l1-ratio", type=float, default=0.5, help="L1 ratio (0=Ridge, 1=Lasso)")
...
def build_elasticnet_pipeline(
    ...
    *,
    ...
    """Build ElasticNet pipeline with SU1/SU5 augmenter, preprocessing, and scaling.

    This is analogous to build_xgb_pipeline but uses ElasticNet and includes
    StandardScaler which is essential for linear models.
    """
    ...
    # StandardScaler is REQUIRED for linear models to ensure proper coefficient interpretation
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Build base pipeline with ElasticNet
    base_pipeline = build_elasticnet_pipeline(
        su1_config,
        su5_config,
        preprocess_settings,
    ...
    final_columns = X_augmented_all.columns.tolist()
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        for step_name in imputer_step_names:
            ...
            if isinstance(X_train, np.ndarray):
                X_train = pd.DataFrame(X_train, columns=pd.Index(final_columns))
            if isinstance(X_valid, np.ndarray):
                X_valid = pd.DataFrame(X_valid, columns=pd.Index(final_columns))

        # Apply StandardScaler
        scaler = clone(pipeline_steps["scaler"])
        X_train = scaler.fit_transform(X_train)  # type: ignore[union-attr]
        X_valid = scaler.transform(X_valid)  # type: ignore[union-attr]
        # Convert back to DataFrame if numpy array
        if isinstance(X_train, np.ndarray):
            X_train = pd.DataFrame(X_train, columns=pd.Index(final_columns))
        if isinstance(X_valid, np.ndarray):
        ...
        # Clone ElasticNet model
        elasticnet_model = clone(pipeline_steps["model"])

        # Fit
        elasticnet_model.fit(X_train, y_train)  # type: ignore[union-attr]

        # Predict
        pred = elasticnet_model.predict(X_valid)  # type: ignore[union-attr]
        pred = _to_1d(pred)

        # Compute metrics
        val_rmse = float(math.sqrt(mean_squared_error(y_valid, pred)))
        train_pred = elasticnet_model.predict(X_train)  # type: ignore[union-attr]
    ...
    if not args.no_artifacts and not args.dry_run:
        ...
        feature_list = {
            "version": f"elasticnet-{args.feature_tier}-v1",
            "created_at": datetime.now(timezone.utc).isoformat(),
        ...
        # The model is the last step in the pipeline
        ...
        coef_df[["feature", "coefficient"]].to_csv(coef_path, index=False)
        print(f"[info] Saved coefficients to {coef_path}")
        
        # Print summary of non-zero coefficients
        non_zero_mask = coef_df["coefficient"] != 0.0
        n_non_zero = non_zero_mask.sum()
        n_total = len(coef_df)
        print(f"[info] Non-zero coefficients: {n_non_zero}/{n_total} ({100*n_non_zero/n_total:.1f}%)")
        print(f"[info] Intercept: {intercept:.6f}")
