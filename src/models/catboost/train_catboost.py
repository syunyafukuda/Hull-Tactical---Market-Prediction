...
This script trains a CatBoost model using the FS_compact feature set (116 features)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="Train CatBoost with FS_compact features.")
    ap.add_argument(
        "--data-dir",
        type=str,
        default="data/raw",
    ...
    ap.add_argument(
        "--out-dir",
        type=str,
        default="artifacts/models/catboost",
    )
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
...
def build_catboost_pipeline(
    ...
    *,
    ...
    """Build CatBoost pipeline with SU1/SU5 augmenter and preprocessing.

    This is analogous to build_pipeline in train_su5.py but uses CatBoostRegressor
    ...
    if not HAS_CATBOOST or CatBoostRegressor is None:
        raise RuntimeError("CatBoost is required but not installed. Please install 'catboost'.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Build base pipeline with CatBoost
    base_pipeline = build_catboost_pipeline(
        su1_config,
        su5_config,
        preprocess_settings,
    ...
    # CatBoost handles feature names well, no sanitization needed
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        # Clone CatBoost model
        catboost_model = clone(pipeline_steps["model"])

        # Fit with eval_set for monitoring
        catboost_model.fit(  # type: ignore[union-attr]
            X_train,
            y_train,
        ...
        # Predict
        pred = catboost_model.predict(X_valid)  # type: ignore[union-attr]
        pred = _to_1d(pred)

        # Compute metrics
        val_rmse = float(math.sqrt(mean_squared_error(y_valid, pred)))
        train_pred = catboost_model.predict(X_train)  # type: ignore[union-attr]
