#!/usr/bin/env python
"""RandomForest training script using the unified model framework.

This script trains a RandomForest model using the FS_compact feature set (116 features)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="Train RandomForest with FS_compact features.")
    ap.add_argument(
        "--data-dir",
        type=str,
        default="data/raw",
    ...
    ap.add_argument("--oob-score", action="store_true", default=True)
...
def build_rf_pipeline(
    ...
    *,
    ...
    """Build RandomForest pipeline with SU1/SU5 augmenter and preprocessing.

    This is analogous to build_pipeline in train_su5.py but uses RandomForestRegressor
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Build base pipeline with RandomForest
    base_pipeline = build_rf_pipeline(
        su1_config,
        su5_config,
        preprocess_settings,
    ...
    final_feature_columns = X_augmented_all.columns.tolist()
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        for step_name in imputer_step_names:
            ...
            if isinstance(X_train, np.ndarray):
                X_train = pd.DataFrame(X_train, columns=pd.Index(final_feature_columns))
            if isinstance(X_valid, np.ndarray):
                X_valid = pd.DataFrame(X_valid, columns=pd.Index(final_feature_columns))

        # Clone RandomForest model
        rf_model = clone(pipeline_steps["model"])

        # Fit model
        rf_model.fit(X_train, y_train)  # type: ignore[union-attr]

        # Store feature importances
        feature_importances_list.append(rf_model.feature_importances_)  # type: ignore[union-attr]

        # Print OOB score if available
        if args.oob_score and hasattr(rf_model, "oob_score_"):
            oob_score = rf_model.oob_score_  # type: ignore[union-attr]
            print(f"[fold {fold_idx}] OOB R^2 Score: {oob_score:.6f}")

        # Predict
        pred = rf_model.predict(X_valid)  # type: ignore[union-attr]
        pred = _to_1d(pred)

        # Compute metrics
        val_rmse = float(math.sqrt(mean_squared_error(y_valid, pred)))
        train_pred = rf_model.predict(X_train)  # type: ignore[union-attr]
    ...
    if not args.no_artifacts and not args.dry_run:
        ...
        # Retrain on all data
        final_pipeline = build_rf_pipeline(
            su1_config,
            su5_config,
            preprocess_settings,
            numeric_fill_value=args.numeric_fill_value,
        ...
        model_input_columns = final_feature_columns
        
        feature_list = {
            "version": f"randomforest-{args.feature_tier}-v1",
            "created_at": datetime.now(timezone.utc).isoformat(),
