...
This script trains a Ridge regression model (L2-regularized linear model) using the
FS_compact feature set (116 features) and provides OOF evaluation consistent with
other model types.
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap = argparse.ArgumentParser(description="Train Ridge with FS_compact features.")
    ap.add_argument(
        "--data-dir",
        type=str,
        default="data/raw",
    ...
    ap.add_argument(
        "--out-dir",
        type=str,
        default="artifacts/models/ridge",
    )
    ap.add_argument(
        "--numeric-fill-value",
        type=float,
    ...
    ap.add_argument("--alpha", type=float, default=1.0, help="L2 regularization strength")
    ap.add_argument("--fit-intercept", action="store_true", default=True, help="Fit intercept")
    ap.add_argument("--solver", type=str, default="auto", help="Solver to use")
    ap.add_argument("--max-iter", type=int, default=None, help="Maximum iterations")
    ap.add_argument("--tol", type=float, default=0.0001, help="Tolerance for optimization")
...
def build_ridge_pipeline(
    ...
    *,
    ...
    """Build Ridge pipeline with SU1/SU5 augmenter and preprocessing.

    This pipeline includes StandardScaler which is essential for Ridge regression
    ...
    # StandardScaler: CRITICAL for Ridge regression
    scaler = StandardScaler()

    # Ridge model
    model = Ridge(**model_kwargs)
    ...
    steps = [
        ...
        ("scaler", scaler),  # StandardScaler before Ridge
        ("model", model),
    ]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.tol is not None:
        model_kwargs["tol"] = args.tol

    # Build base pipeline with Ridge
    base_pipeline = build_ridge_pipeline(
        su1_config,
        su5_config,
        preprocess_settings,
    ...
    # Save final column names (no sanitization needed for Ridge)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(splitter.split(X_np), start=1):
        ...
        # Manual preprocessing: fit on train, transform both train and valid
        ...
        # Clone Ridge model
        ridge_model = clone(pipeline_steps["model"])

        # Fit model
        ridge_model.fit(X_train, y_train)  # type: ignore[union-attr]

        # Predict
        pred = ridge_model.predict(X_valid)  # type: ignore[union-attr]
        pred = _to_1d(pred)

        # Compute metrics
        val_rmse = float(math.sqrt(mean_squared_error(y_valid, pred)))
        train_pred = ridge_model.predict(X_train)  # type: ignore[union-attr]
    ...
    if not args.no_artifacts and not args.dry_run:
        ...
        feature_list = {
            "version": f"ridge-{args.feature_tier}-v1",
            "created_at": datetime.now(timezone.utc).isoformat(),
