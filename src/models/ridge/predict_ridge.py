...
def _resolve_postprocess_params(
    ...
    *,
    ...
    # Try oof_best_params first (Ridge format), then post_process_params
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with the Ridge pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory storing Ridge artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run train_ridge.py first."
        )
    ...
    # Ridge bundle is a direct Pipeline (not a dict)
