...
"""
MSR-proxy 推論・提出スクリプト (D 系特徴量版):
- data/raw/{test.parquet|test.csv} を自動検出（--test-file で明示可）
- artifacts/Preprocessing_D/model_pre_d.pkl と model_meta.json をロード
...
from preprocess.E_group.e_group import DGroupImputer  # noqa: F401,E402
...
def main() -> int:
    ...
    ap.add_argument("--artifacts-dir", type=str, default="artifacts/Preprocessing_D")
    ap.add_argument("--id-col", type=str, default="date_id")
    ap.add_argument("--pred-col", type=str, default="prediction")
    ap.add_argument("--out-parquet", type=str, default="artifacts/Preprocessing_D/submission.parquet")
    ap.add_argument("--out-csv", type=str, default="artifacts/Preprocessing_D/submission.csv", help="also write csv (default: artifacts/Preprocessing_D/submission.csv)")
    ...
    model_path = art_dir / "model_pre_d.pkl"
    meta_path = art_dir / "model_meta.json"

    if not model_path.exists():
        raise FileNotFoundError(f"model not found: {model_path}. Run train_pre_d.py first.")
    ...
    if feature_list_spec is None:
        feature_list_spec = {
            ...
            "calendar_column": meta.get("d_calendar_col"),
            "d_columns": meta.get("d_columns", []),
            "d_generated_columns": meta.get("d_generated_columns", []),
    ...
    calendar_col = feature_list_spec.get("calendar_column") or meta.get("d_calendar_col")
    ...
    d_columns_meta = list(feature_list_spec.get("d_columns", []))
    if not d_columns_meta:
        d_columns_meta = [c for c in feat_cols if isinstance(c, str) and c.startswith("D")]
    ...
    imputer_loaded = pipe.named_steps.get("d_imputer")
    if imputer_loaded is None:
        raise KeyError("d_imputer step missing in loaded pipeline. Re-train artifacts.")
    trained_policy = getattr(imputer_loaded, "policy_requested", None)
    if trained_policy and meta.get("d_policy") and trained_policy != meta.get("d_policy"):
        raise ValueError(
            f"Loaded pipeline policy '{trained_policy}' does not match meta recorded policy '{meta.get('d_policy')}'."
        )
    trained_columns = list(getattr(imputer_loaded, "columns_", []) or [])
    if d_columns_meta:
        if trained_columns and set(trained_columns) != set(d_columns_meta):
            raise ValueError(
                "Mismatch between meta-recorded D columns and the fitted imputer columns."
            )
    elif trained_columns:
        raise ValueError("Model exposes D columns but metadata is missing d_columns definition.")
    ...
    for col in required_columns:
        ...
        if col in numeric_cols or col in d_columns_meta:
            df_feat[col] = np.nan
    ...
    if d_columns_meta:
        df_feat.loc[:, d_columns_meta] = df_feat.loc[:, d_columns_meta].apply(pd.to_numeric, errors="coerce")
    if m_columns_meta:
        df_feat.loc[:, m_columns_meta] = df_feat.loc[:, m_columns_meta].apply(pd.to_numeric, errors="coerce")

    # 統一された列順を構築
    df_model_input = df_feat.loc[:, required_columns]
    df_with_id = pd.concat([df_test[[args.id_col]].copy(), df_model_input], axis=1)
    df_with_id["_idx"] = np.arange(len(df_with_id))
    df_sorted = df_with_id.sort_values(args.id_col).reset_index(drop=True)
    ...
    # 元の順序へ戻す
    out_df = pd.DataFrame({"_idx": df_sorted["_idx"].values, args.id_col: df_sorted[args.id_col].values, args.pred_col: pred_out})
    ...
    policy_version = {
        "d": {
            "name": meta.get("d_policy"),
            "params": meta.get("d_policy_params"),
    ...
    pred_meta = {
        ...
        "d_columns": d_columns_meta,
        "d_generated_columns": feature_list_spec.get("d_generated_columns", []),
        "d_all_nan_columns": meta.get("d_all_nan_columns", []),
        "d_all_nan_strategy": meta.get("d_all_nan_strategy"),
        "d_all_nan_fill_value": meta.get("d_all_nan_fill_value"),
