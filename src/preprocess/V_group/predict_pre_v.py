...
"""
Preprocessing-V 推論・提出スクリプト:
- data/raw/{test.parquet|test.csv} を自動検出（--test-file で明示可）
- artifacts/Preprocessing_V/{model_pre_v.pkl, model_meta.json, feature_list.json} をロード
...
def main() -> int:
    ...
    ap.add_argument("--artifacts-dir", type=str, default="artifacts/Preprocessing_V")
    ap.add_argument("--id-col", type=str, default="date_id")
    ap.add_argument("--pred-col", type=str, default="prediction")
    ap.add_argument("--out-parquet", type=str, default="artifacts/Preprocessing_V/submission.parquet")
    ap.add_argument("--out-csv", type=str, default="artifacts/Preprocessing_V/submission.csv", help="also write csv (default: artifacts/Preprocessing_V/submission.csv)")
    ...
    model_path = art_dir / "model_pre_v.pkl"
    meta_path = art_dir / "model_meta.json"
    feature_spec_path = art_dir / "feature_list.json"

    if not model_path.exists():
        raise FileNotFoundError(f"model not found: {model_path}. Run train_pre_v.py first.")
    ...
    if not feat_cols:
        raise KeyError("feature_columns not found in artifacts. Re-train with updated train_pre_v.py")
    ...
    def _normalize_columns(raw: Any) -> list[str]:
        ...
        if isinstance(raw, Sequence):
            return [str(col) for col in raw if col]
    ...
    numeric_cols = set(_normalize_columns(spec_numeric))
    cat_cols = set(_normalize_columns(spec_categorical))
    m_columns = _normalize_columns(feature_spec.get("m_columns") or meta.get("m_columns"))
    s_columns = _normalize_columns(feature_spec.get("s_columns") or meta.get("s_columns"))
    v_columns = _normalize_columns(feature_spec.get("v_columns") or meta.get("v_columns"))
    e_columns = _normalize_columns(feature_spec.get("e_columns") or meta.get("e_columns"))
    i_columns = _normalize_columns(feature_spec.get("i_columns") or meta.get("i_columns"))
    p_columns = _normalize_columns(feature_spec.get("p_columns") or meta.get("p_columns"))
    ...
    # Numeric sanitisation mirrors training for M/E/I/P/S/V feature groups.
    for group_cols in (m_columns, e_columns, i_columns, p_columns, s_columns, v_columns):
        for col in group_cols:
            if col in model_input.columns:
                model_input.loc[:, col] = pd.to_numeric(model_input[col], errors="coerce")

    # Coerce remaining numeric features declared in metadata (e.g., auxiliary columns) to numeric dtype.
    ...
    order_idx_series = id_series.argsort(kind="mergesort")
    order_idx = np.asarray(order_idx_series, dtype=np.int64)
    X_infer = model_input.iloc[order_idx]
    ...
    if use_pp:
        ...
        pred_sorted = signal.astype(float)
        print(f"[info] applied post-process: mult={params.mult} lo={params.lo} hi={params.hi}")
    else:
        pred_sorted = yhat.astype(float)

    inv_idx = np.empty_like(order_idx)
    inv_idx[order_idx] = np.arange(order_idx.shape[0])
    pred_out = np.asarray(pred_sorted, dtype=float)[inv_idx]

    sub = pd.DataFrame({args.id_col: df_test[args.id_col].values, args.pred_col: pred_out})
