...
- 学習済みモデルとメタ情報（MSR関連を含む）を artifacts/Preprocessing_I/ に保存
"""
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", type=str, default="artifacts/Preprocessing_I")
    ...
    if m_calendar_col is None:
        m_calendar_col = e_calendar_col or i_calendar_col

    calendar_columns: List[str] = []
    for col in (i_calendar_col, e_calendar_col, m_calendar_col):
    ...
    mask_cols = list(dict.fromkeys(e_mask_cols + i_mask_cols))
    ...
    model_path = out_dir / "model_pre_i.pkl"
