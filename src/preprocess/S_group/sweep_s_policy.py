#!/usr/bin/env python
"""S 系欠損補完ポリシーを横断的に実行し、CV 指標を集計するスクリプト。"""
...
TRAIN_SCRIPT = THIS_DIR / "train_pre_s.py"
...
SUPPORTED_POLICIES = list(SGroupImputer.SUPPORTED_POLICIES)
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=str, default="artifacts/Preprocessing_S")
    parser.add_argument("--results-dir", type=str, default="results/ablation/S_group")
    parser.add_argument(
        "--suite",
        dest="suites",
        action="append",
    ...
    parser.add_argument("--s-rolling-window", type=int, default=5, help="Window size forwarded to train_pre_s.py")
    parser.add_argument("--s-ema-alpha", type=float, default=0.3, help="EMA alpha forwarded to train_pre_s.py")
    parser.add_argument(
        "--calendar-col",
        type=str,
        default="date_id",
        help="Fallback calendar column forwarded to train_pre_s.py when explicit overrides are not provided (empty string to disable).",
    )
    parser.add_argument(
        "--s-calendar-col",
        type=str,
        default=None,
        help="Optional override for the S policy calendar column (default: reuse --calendar-col)",
    )
    parser.add_argument(
        "--e-calendar-col",
        type=str,
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Additional S policy parameters passed as key=value (repeat option).",
    )
    parser.add_argument(
        "--i-policy",
        type=str,
        default=None,
        help="Optional override for the I policy executed before each S run.",
    )
    parser.add_argument("--i-rolling-window", type=int, default=None, help="Override for --i-rolling-window passed to train_pre_s.py.")
    parser.add_argument("--i-ema-alpha", type=float, default=None, help="Override for --i-ema-alpha passed to train_pre_s.py.")
    parser.add_argument(
        "--i-policy-param",
        action="append",
        default=[],
    ...
    parser.add_argument("--i-clip-low", type=float, default=None, help="Lower quantile forwarded to train_pre_s.py for I clipping.")
    parser.add_argument("--i-clip-high", type=float, default=None, help="Upper quantile forwarded to train_pre_s.py for I clipping.")
    parser.add_argument("--disable-i-clip", action="store_true", help="Disable quantile clipping for I columns during sweep runs.")
    parser.add_argument(
        "--e-policy-param",
        action="append",
        default=[],
        help="Additional E policy parameters forwarded to train_pre_s.py (repeatable).",
    )
    parser.add_argument(
        "--m-policy-param",
        action="append",
        default=[],
        help="Additional M policy parameters forwarded to train_pre_s.py (repeatable).",
    )
    parser.add_argument("--extra-args", type=str, default="", help="additional args passed to train_pre_s.py")
    parser.add_argument(
        "--pp-aggregate",
        type=str,
        default="refit",
        choices=["refit", "median", "vote"],
        help="post-process aggregation strategy passed to train_pre_s.py",
    )
    parser.add_argument(
        "--s-mad-scale",
        type=float,
        default=4.0,
        help="MAD scaling coefficient forwarded to train_pre_s.py",
    )
    parser.add_argument(
        "--s-mad-min-samples",
        type=int,
        default=25,
        help="Minimum samples required before applying MAD clipping (forwarded to train_pre_s.py).",
    )
    parser.add_argument(
        "--disable-s-mad-clip",
        action="store_true",
        help="Disable MAD/quantile clipping for S columns during sweep runs.",
    )
    parser.add_argument(
        "--s-fallback-quantile-low",
        type=float,
        default=0.005,
        help="Lower quantile used when MAD clip is unavailable (forwarded to train_pre_s.py).",
    )
    parser.add_argument(
        "--s-fallback-quantile-high",
        type=float,
        default=0.995,
        help="Upper quantile used when MAD clip is unavailable (forwarded to train_pre_s.py).",
    )
    ...
    parser.add_argument(
        ...
        help="Optional override for the P policy executed before each S run.",
    )
    parser.add_argument(
        "--p-rolling-window",
        type=int,
        default=None,
        help="Override for --p-rolling-window passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--p-ema-alpha",
        type=float,
        default=None,
        help="Override for --p-ema-alpha passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--p-policy-param",
        action="append",
        default=[],
        help="Additional P policy parameters forwarded to train_pre_s.py (repeatable).",
    )
    parser.add_argument(
        "--p-calendar-col",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override for --p-mad-scale passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--p-mad-min-samples",
        type=int,
        default=None,
        help="Override for --p-mad-min-samples passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--p-fallback-quantile-low",
        type=float,
        default=None,
        help="Override for --p-fallback-quantile-low passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--p-fallback-quantile-high",
        type=float,
        default=None,
        help="Override for --p-fallback-quantile-high passed to train_pre_s.py.",
    )
    parser.add_argument(
        "--disable-p-mad-clip",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Optional override for the M policy executed before each S run.",
    )
    parser.add_argument("--m-rolling-window", type=int, default=None, help="Override for --m-rolling-window passed to train_pre_s.py.")
    parser.add_argument("--m-ema-alpha", type=float, default=None, help="Override for --m-ema-alpha passed to train_pre_s.py.")
    parser.add_argument(
        "--e-policy",
        type=str,
        default=None,
        choices=sorted(EGroupImputer.SUPPORTED_POLICIES),
        help="Optional override for the E policy executed before each S run.",
    )
    parser.add_argument("--e-rolling-window", type=int, default=None, help="Override for --e-rolling-window passed to train_pre_s.py.")
    parser.add_argument("--e-ema-alpha", type=float, default=None, help="Override for --e-ema-alpha passed to train_pre_s.py.")
    parser.add_argument("--max-workers", type=int, default=1, help="Number of concurrent policy runs (default: 1).")
    parser.add_argument(
        "--model-n-jobs",
        type=int,
        default=None,
        help="Override LightGBM n_jobs passed to train_pre_s.py (default: -1, forced to 1 when --max-workers>1 unless set)",
    )
    parser.add_argument("--timeout", type=int, default=None, help="Timeout in seconds for each train_pre_s.py invocation.")
    ...
    parser.add_argument("--random-seed", type=int, default=42, help="Random seed forwarded to train_pre_s.py")
    ...
    s_policy_param_dict = parse_policy_param_options(args.s_policy_param or [])
    s_policy_param_cli = serialize_policy_params(s_policy_param_dict)
    ...
    supported_s_policies = set(SGroupImputer.SUPPORTED_POLICIES)
    unsupported = [policy for policy in selected_policies if policy not in supported_s_policies]
    if unsupported:
        print(f"[warn] unsupported policies requested: {unsupported} -> they will be skipped")
        selected_policies = [policy for policy in selected_policies if policy in supported_s_policies]
    if not selected_policies:
    ...
    if s_policy_param_dict:
        print(f"[info] S policy param overrides: {s_policy_param_dict}")
    ...
    def summarize_result(
        ...
        row = {
            "policy": metrics.get("s_policy", policy),
            "status": status,
            "s_rolling_window": metrics.get("s_rolling_window", fallback.get("s_rolling_window")),
            "s_ema_alpha": metrics.get("s_ema_alpha", fallback.get("s_ema_alpha")),
            "s_calendar_col": metrics.get("s_calendar_col", fallback.get("s_calendar_col")),
            ...
            "s_post_impute_nan_ratio": metrics.get("s_post_impute_nan_ratio"),
            ...
            "s_imputer_warning_count": metrics.get("s_imputer_warning_count"),
            "s_imputer_warnings": metrics.get("s_imputer_warnings"),
            "p_imputer_warning_count": metrics.get("p_imputer_warning_count"),
            "p_imputer_warnings": metrics.get("p_imputer_warnings"),
            "s_mad_scale": metrics.get("s_mad_scale", fallback.get("s_mad_scale")),
            "s_mad_min_samples": metrics.get("s_mad_min_samples", fallback.get("s_mad_min_samples")),
            "s_mad_clip_enabled": metrics.get("s_mad_clip_enabled", fallback.get("s_mad_clip_enabled")),
            "s_fallback_quantile_low": metrics.get("s_fallback_quantile_low", fallback.get("s_fallback_quantile_low")),
            "s_fallback_quantile_high": metrics.get("s_fallback_quantile_high", fallback.get("s_fallback_quantile_high")),
        ...
        s_columns = metrics.get("s_columns")
        s_column_count = metrics.get("s_column_count")
        if s_column_count is None and isinstance(s_columns, list):
            s_column_count = len(s_columns)
        row["s_column_count"] = s_column_count
        ...
        for k, v in (metrics.get("s_policy_params") or {}).items():
            row[f"s_param_{k}"] = v
        ...
        _collect_clip_stats("s", metrics.get("s_clip_bounds"))
        _collect_clip_stats("p", metrics.get("p_clip_bounds"))

        return row

    def execute_policy(policy: str) -> dict[str, Any]:
        metrics_path = results_dir / f"{timestamp}_s_group_{policy}.json"
        computed_model_n_jobs = (
            args.model_n_jobs if args.model_n_jobs is not None else (1 if args.max_workers > 1 else -1)
        )
        effective_s_calendar = args.s_calendar_col if args.s_calendar_col is not None else args.calendar_col
        ...
        elif effective_s_calendar is not None:
            effective_m_calendar = effective_s_calendar
        else:
        ...
        if metrics_path.exists():
            if args.resume:
                ...
                fallback = {
                    "s_rolling_window": args.s_rolling_window,
                    "s_ema_alpha": args.s_ema_alpha,
                    "s_calendar_col": effective_s_calendar,
                    "pp_aggregate": args.pp_aggregate,
                    "s_mad_scale": args.s_mad_scale,
                    "s_mad_min_samples": args.s_mad_min_samples,
                    "s_mad_clip_enabled": not args.disable_s_mad_clip,
                    "s_fallback_quantile_low": args.s_fallback_quantile_low,
                    "s_fallback_quantile_high": args.s_fallback_quantile_high,
        ...
        cmd = [
            ...
            policy,
            "--s-rolling-window",
            str(args.s_rolling_window),
            "--s-ema-alpha",
            str(args.s_ema_alpha),
            "--no-artifacts",
            "--metrics-path",
            str(metrics_path),
            "--pp-aggregate",
            ...
            "--s-mad-scale",
            str(args.s_mad_scale),
            "--s-mad-min-samples",
            str(args.s_mad_min_samples),
            "--s-fallback-quantile-low",
            str(args.s_fallback_quantile_low),
            "--s-fallback-quantile-high",
            str(args.s_fallback_quantile_high),
        ]
        if args.disable_s_mad_clip:
            cmd.append("--disable-s-mad-clip")
        if effective_s_calendar:
            cmd.extend(["--s-calendar-col", effective_s_calendar])
        ...
        for param in s_policy_param_cli:
            cmd.extend(["--s-policy-param", param])
        if args.p_policy:
    ...
    summary_path = results_dir / f"{timestamp}_s_group_summary.csv"
    if summary_rows:
        import csv
        ...
        base_fieldnames = [
            ...
            "optimize_for",
            "s_column_count",
            "s_post_impute_nan_ratio",
            "s_mad_scale",
            "s_mad_min_samples",
            "s_mad_clip_enabled",
            "s_fallback_quantile_low",
            "s_fallback_quantile_high",
            "p_column_count",
            ...
            "pp_best_hi_std",
            "pp_best_lam_std",
            "s_clip_rel_width_mean",
            "s_clip_rel_width_min",
            "s_clip_rel_width_max",
        ...
        stats_path = results_dir / f"{timestamp}_s_group_summary_stats.json"
