...
"""
Preprocessing-S 推論・提出スクリプト:
- data/raw/{test.parquet|test.csv} を自動検出（--test-file で明示可）
- artifacts/Preprocessing_S/{model_pre_s.pkl, model_meta.json, feature_list.json} をロード
...
def main() -> int:
    ...
    ap.add_argument("--artifacts-dir", type=str, default="artifacts/Preprocessing_S")
    ap.add_argument("--id-col", type=str, default="date_id")
    ap.add_argument("--pred-col", type=str, default="prediction")
    ap.add_argument("--out-parquet", type=str, default="artifacts/Preprocessing_S/submission.parquet")
    ap.add_argument("--out-csv", type=str, default="artifacts/Preprocessing_S/submission.csv", help="also write csv (default: artifacts/Preprocessing_S/submission.csv)")
    ...
    model_path = art_dir / "model_pre_s.pkl"
    meta_path = art_dir / "model_meta.json"
    feature_spec_path = art_dir / "feature_list.json"

    if not model_path.exists():
        raise FileNotFoundError(f"model not found: {model_path}. Run train_pre_s.py first.")
    ...
    if not feat_cols:
        raise KeyError("feature_columns not found in artifacts. Re-train with updated train_pre_s.py")
    ...
    # Numeric sanitisation mirrors training for E/I/P/S feature groups.
    for group_cols in (e_columns, i_columns, p_columns, s_columns):
