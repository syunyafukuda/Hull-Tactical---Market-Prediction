...
- 学習済みモデルとメタ情報（MSR関連を含む）を artifacts/Preprocessing_S/ に保存
"""
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", type=str, default="artifacts/Preprocessing_S")
    ...
    ap.add_argument("--m-policy", type=str, default="ridge_stack", choices=list(MGroupImputer.SUPPORTED_POLICIES), help="M group imputer policy applied prior to E/I/S policies (default: ridge_stack).")
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Optional calendar column override for the M policy (default: follow S/I/E precedence).",
    )
    ...
    ap.add_argument(
        ...
        help="P group imputer policy applied between I and S stages.",
    )
    ...
    ap.add_argument(
        ...
        help="S group imputer policy applied last in the preprocessing chain.",
    )
    ...
    if m_calendar_col is None:
        for candidate in (p_calendar_col, s_calendar_col, i_calendar_col, e_calendar_col):
            if candidate is not None:
                m_calendar_col = candidate
                break

    calendar_columns: List[str] = []
    for col in (p_calendar_col, s_calendar_col, i_calendar_col, e_calendar_col, m_calendar_col):
    ...
    mask_cols = list(dict.fromkeys(e_mask_cols + i_mask_cols + p_mask_cols + s_mask_cols))
    ...
    for train_idx, val_idx in tscv.split(X_np):
        ...
        s_missing_ratio_fold: Dict[str, float] = {}
        for col in s_cols:
            if col not in X_va.columns:
                continue
            converted = cast(pd.Series, pd.to_numeric(X_va[col], errors="coerce"))
            s_missing_ratio_fold[col] = float(converted.isna().mean())
        ...
        fold_logs.append(
            {
                ...
                **{f"best_{k}": v for k, v in best_metrics.items()},
                ...
                "s_missing_ratio": s_missing_ratio_fold,
            }
        )
        log_msg = (
    ...
    try:
        if s_imputer_fitted is not None:
            ...
            if isinstance(state_dict_s, dict):
                ...
                clip_bounds_state_s = state_dict_s.get("mad_clip_bounds")
    ...
    model_path = out_dir / "model_pre_s.pkl"
