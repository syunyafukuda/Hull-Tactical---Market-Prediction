...
学習済みの ``artifacts/tier1/inference_bundle.pkl`` を読み込み、指定されたテストデータに
対して推論を実行し、提出形式の ``submission.csv`` を出力する。

Usage:
    python -m src.feature_selection.predict_tier --artifacts-dir artifacts/tier1
"""
...
def _resolve_postprocess_params(
    ...
    *,
    ...
    # Get from post_process_params in metadata
    pp_params = meta.get("post_process_params", {})
    mult = pp_params.get("best_mult", 1.0)
    lo = pp_params.get("best_lo", 0.9)
    hi = pp_params.get("best_hi", 1.1)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with the Tier pipeline bundle."
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory storing tier artifacts",
    )
    parser.add_argument(
        "--bundle-path",
        type=str,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    if not bundle_path.exists():
        raise FileNotFoundError(
            f"inference bundle not found: {bundle_path}. Run evaluate_baseline.py first."
        )
    ...
    # Handle bundle structure: may contain augmenter + pipeline or just pipeline
    if isinstance(bundle, dict):
        augmenter = bundle.get("augmenter")
        pipeline = bundle.get("pipeline")
        print(f"[info] bundle contains augmenter: {augmenter is not None}")
    else:
        # Assume bundle is the pipeline directly
        augmenter = None
        pipeline = bundle
    
    # Apply augmenter (SU1 + SU5) if present
    if augmenter is not None:
        print("[info] applying feature augmenter (SU1 + SU5)...")
        X_augmented = augmenter.transform(X_test)
        print(f"[info] augmented shape: {X_augmented.shape}")
    else:
        X_augmented = X_test
