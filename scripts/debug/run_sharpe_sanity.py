...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--holdout-size", type=int, default=1000,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    positions = map_to_position(
        y_pred,
        multiplier=args.sharpe_mult,
        offset=args.sharpe_offset,
    )
    ...
    if args.verbose:
        ...
        print(f"        pct_at_min={pos_stats['pct_at_min_clip']*100:.1f}%, pct_at_max={pos_stats['pct_at_max_clip']*100:.1f}%")
    ...
    if result.vol_penalty >= 0 and result.return_penalty >= 0:
        ...
        print(f"[FAIL] Negative penalty: vol={result.vol_penalty}, ret={result.return_penalty}")
    ...
    if positions.min() >= 0 and positions.max() <= 2:
        ...
        print(f"[FAIL] Positions out of range: min={positions.min()}, max={positions.max()}")
