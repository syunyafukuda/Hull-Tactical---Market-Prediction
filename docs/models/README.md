...
| モデル | OOF RMSE | LB Score | 対 LGBM 予測相関 | 予測 Std | アンサンブル価値 | ステータス |
|--------|----------|----------|-----------------|---------|------------------|----------|
| **LightGBM** | 0.012164 | **0.622** | - | 0.005246 | ベースライン | ✅ 完了 |
| **XGBoost** | **0.012062** | 0.618 | 0.89 | 0.004999 | ✅ **高い** | ✅ 完了 |
| **CatBoost** | 0.011095 | **0.602** ❌ | 0.35 | 0.000495 | ❌ **低い** | ✅ 完了 |
| Ridge | - | - | - | - | TBD | ⬜ 未着手 |
| Lasso | - | - | - | - | TBD | ⬜ 未着手 |
| ElasticNet | - | - | - | - | TBD | ⬜ 未着手 |
...
XGBoostは単体LBが0.618とLGBMより僅かに劣るが、**予測相関が0.89と適度に低くアンサンブル価値が高い**。

### CatBoost 分析結果

**致命的な問題点:**
- LB Score 0.602はLGBM比で-3.2%と大幅悪化
- 予測分散が0.000495とLGBM（0.005246）の9%しかない
- 市場の実際の変動を全く捉えられていない（予測が極端に平坦化）

**原因分析:**
- CatBoostのSymmetric TreeとOrdered Boostingが本データセットに過度に保守的
- ハイパーパラメータ調整（depth, l2_leaf_reg, learning_rate, boosting_type）でも改善不可
- LGBMとの相関0.35、XGBoostとの相関0.27は異なるパターンを学習しているが有用でない

**結論:**
- ❌ 単独採用は見送り
- ⚠️ アンサンブルでの採用も慎重に。予測の平坦化がアンサンブル全体を劣化させるリスク大
- 💡 代替として、LGBM/XGBoostの異なるハイパーパラメータでの複数モデル作成を推奨
...
| ExtraTrees | 0.85-0.92 | TBD | 高（バギング系で揺れ方が異なる） |
| RandomForest | 0.85-0.92 | TBD | 高（バギング系） |
| Ridge | 0.70-0.85 | TBD | 非常に高い（線形 vs 非線形） |
| Lasso | 0.70-0.85 | TBD | 非常に高い（線形・スパース） |
| ElasticNet | 0.70-0.85 | TBD | 非常に高い（線形・ハイブリッド） |
