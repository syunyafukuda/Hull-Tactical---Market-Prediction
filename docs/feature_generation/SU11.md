...
|------|------|
| 実装状況 | **実装完了・評価済み** |
| 採否ステータス | `status: not_adopted`, `decision: rejected` |
| LB スコア | **0.464** ❌ (ベースライン SU5: 0.681 から **-31.9% 大幅悪化**) |
...
| 指標 | SU5 (Level-1) | SU11 Ridge (Level-2) | 変化 |
|------|---------------|----------------------|------|
...
| 対策 | 結果 |
|------|------|
| alpha=1.0 → 0.001 | coef が 0.014 → 0.078 に改善、但し依然 shrinkage 過大 |
| alpha=0.00001 | coef が 0.078 に収束（これ以上改善せず） |
| identity モード | Level-1 をそのまま通過 → 実質 SU5 と同等 |

**結論**: 単一 Level-1 入力での Ridge stacking は、shrinkage により予測分散を失い、
LB スコアを大幅に悪化させる。本アプローチは本コンペでは無効。
...
```yaml
su11:
  # ❌ SU11 非採用 (2025-12-06決定)
  # 理由: LB 0.464 (ベースラインSU5の0.681から-31.9%大幅悪化)
  # OOF RMSE: 0.010997 (ベースライン0.012139から-9.4%改善だが、LBでは逆効果)
  # 原因: Level-2 Ridgeが予測を97%縮小し、ベッティングシグナルを消失
  # 結論: 単一モデルstackingは本コンペでは無効、SU5をベストラインとして継続
  enabled: false
  mode: "stacking"
  level1_artifacts_dir: "artifacts/SU5"
  level2_model_type: "ridge"  # identity も検証済み
  ...
  level2_params:
    ridge:
      alpha: 0.001  # 1.0 → 0.001 に調整したが効果なし
    lgbm:
      n_estimators: 50
      max_depth: 3
      learning_rate: 0.05
  ...
  metadata:
    ...
    lb_change_pct: -31.9
    oof_rmse: 0.010997
    baseline_oof_rmse: 0.012139
    oof_change_pct: -9.4  # 改善
    prediction_shrinkage: 97.3  # 予測分散の縮小率
    ridge_coef: 0.0137
    evaluation_date: "2025-12-06"
    notes: |
      ❌ 非採用。LB 0.464（ベースライン0.681から-31.9%大幅悪化）。
      OOF RMSE は 9.4% 改善したが、Level-2 Ridge が予測を 97% 縮小。
      結果としてベッティングシグナルが消失し、ほぼ「常に1.0を予測」する状態に。
      単一 Level-1 入力での stacking は本コンペでは無効と判断。
      真の改善には複数の多様な Level-1 モデル (SU5, SU7, SU8等) のアンサンブルが必要。
      SU1+SU5（LB 0.681）を引き続きベストラインとして維持。
```
