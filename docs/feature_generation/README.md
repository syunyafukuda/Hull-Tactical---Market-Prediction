...
- **SU5_共欠損・交差** ✅ **実装完了（コア機能）**
	...
	- 特徴量数: top_k_pairs × (1 + len(windows)) + degree 列
		- 例: 10 pairs × (1 + 2 windows) + degree = 33 列程度
	- 実装状況:
		- ✅ コア機能完成 (`feature_su5.py`, `SU5Config`, `SU5FeatureGenerator`, `SU5FeatureAugmenter`)
		- ✅ 単体テスト 7 件全通過、83% カバレッジ
		- ✅ 品質チェック全通過 (Ruff, Pyright, Pytest)
		- ✅ Pipeline scripts (train/predict/sweep) 構造実装
		- 📋 実データ統合待ち (full pipeline execution)
	- 提出可否: **評価待ち** (OOF/LB検証必要、目標: ΔMSR ≥ +0.5σ)
	- 備考: top-k ペア選択で特徴量爆発を防止、fold 境界リセット対応
- **SU6_圧縮（必要時のみ）** ⏸ **当面スキップ**
	- 内容: 群内欠損フラグ行列 PCA `miss_pca1..k`（`k≤3`, fold 内 fit）
	- ブランチ: `feat/miss-pca-k2-su6`（Issue/設計は残すが、当面は実装しない）
	- 提出可否: 特徴爆発時のみ適用し `ΔMSR` 非劣化を確認する想定（現状は未実装）
	- 備考: コンポーネントは学習折のみに fit。**SU7〜SU9（欠損以外の軸）の実装・評価が一通り終わり、総特徴量数に問題が出た場合の「最後の手段」として再検討する。**
	- 今後の拡張: 欠損軸に加えて、**価格変化軸（SU7）、ボラ・レジーム軸（SU8）、カレンダー軸（SU9）** を先に追加し、その後に必要なら SU6 による圧縮を検討する。

### SU7〜SU9 の共通方針（欠損以外の軸）

- 役割:
	- **SU7**: モメンタム & リバーサル（価格・リターン系列の値動きパターン）
	- **SU8**: ボラティリティ & レジーム（低/中/高ボラ、トレンド状態）※本コンペでは非採用
	- **SU9**: カレンダー & 季節性（曜日・月・祝日・月末/期末など時間構造）
- パイプライン上の位置:
	- 生データ → SU1 → SU5 → GroupImputers（M/E/I/P/S）
	→ **SU7–SU9（欠損補完後の派生特徴）** → 前処理（スケーラー＋OneHot）→ LightGBM
	- SU7〜SU9 は **SU1/SU5 + GroupImputers による欠損補完後の特徴行列を入力**とし、その上に追加特徴を作る（生データには直接作用しない）。
- 列数上限（実効値の目安）:
	- SU7: +40〜80 列（対象列 6〜8 本、diff/roll/RSI/sign のコアセット）
	- SU8: +20〜50 列（代表インデックス 1〜2 本に対するボラ指標とレジームタグ）
	- SU9: +20〜40 列（カレンダー one-hot + 祝日/bridge + 年内ポジション）
	- 生+SU1+SU5 ≈ 567 列と合わせて、**最終 ~650〜750 列** を上限とする。
- 導入順と PoC 単位:
	1. Step1: SU9（カレンダーのみ）
	2. Step2: SU7（diff/ローリング/RSI のコア部分）
	3. Step3: SU8（ボラ・レジームタグとボラ調整リターン）
	   - 2025-12-04 時点: SU1+SU5 ベースラインに SU8 を追加したラインを Kaggle に Submit した結果、
	     Public LB は **0.624** となり、SU5 単体ラインの 0.681 から -0.057 ポイント悪化した。
	   - OOF RMSE も SU5 baseline (0.012139) → SU8 追加ライン (0.012230) とわずかに悪化しており、
	     train+OOF/公開期間の両面で優位性が確認できなかったため、SU8 は **非採用（not_adopted）** とした。
