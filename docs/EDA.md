...
補足: D 系特徴量（プレフィクス `D`）は train/test いずれも欠損が検出されず、欠損補完などの前処理は不要であると確認済み。
...
- 欠損: 初期期間に多く、プレフィックス群で似た欠損パターン。D 系特徴量は train/test ともに欠損ゼロで補完不要。連続値はロバスト補完、二値は欠損フラグ併用が有効。
